...
data class Point(
    ...
    companion object {
        fun fromLngLat(lngLat: LngLat): Point {
            return Point(coordinates = listOf(lngLat.longitude, lngLat.latitude))
        }
    }
}
...
data class LineString(
    ...
    fun toLngLats(): List<LngLat> {
        return coordinates.map { LngLat(it[0], it[1]) }
    }
    
    companion object {
        fun fromLngLats(points: List<LngLat>): LineString {
            return LineString(coordinates = points.map { listOf(it.longitude, it.latitude) })
        }
    }
}
...
data class LatLngBounds(
    ...
    fun contains(lngLat: LngLat): Boolean {
        return lngLat.longitude in southwest.longitude..northeast.longitude &&
